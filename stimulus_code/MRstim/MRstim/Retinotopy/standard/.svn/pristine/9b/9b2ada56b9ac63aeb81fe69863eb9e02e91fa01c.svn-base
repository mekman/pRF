function [stimulus otherSequence] = makeRetinotopyStimulus_bars_ecc_scaled(params)
...
%     end
    ...
    step_startx = logspace(log10(barWidth./2),log10(outerRad),ceil(step_nx./2))-(barWidth./2);
...
%         end
        ...
        switch params.experiment
            ...
            case {'8 bars (sinewave)','8 bars (LMS)','8 bars (LMS) with blanks'}
                img1        = bk*ones(m,n);
                img2        = img1;
                tmpvar      = sin((x - loX)*numSubRings*(2*pi/barWidth));

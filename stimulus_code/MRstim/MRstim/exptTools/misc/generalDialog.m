...
%			For edit fields, can also make the edit field multiline by
%			adding a number after 'edit' -- e.g., 'edit5' will make it a
%			5-line edit field. This can be useful for editing long text
%			fields. The special 'filename' style creates an edit field for
%			specifying file names, with a 'Browse' button to the right of
%			the field to navigate to a file. By default the 'browse' button
...
for i = 1:nItems
	switch lower( dlg(i).style(1:4) )
		case 'list'
			% listbox, add as many lines as list entries
			dlg(i).nLines = length(dlg(i).list);		
		case {'edit' 'number'}

function mr = mrLoad_me(pth, format, varargin)
...
if strfind(f, '*')
	...
	mr = mrLoad_me(fullfile(p, fileList(1).name), format);
	
	for ii = 2:length(fileList)
		subMr = mrLoad_me(fullfile(p, fileList(ii).name), format);

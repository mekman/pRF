function mr = mrReadNifti_me(pth)
%
% mr = mrReadNifti(pth)
%
...
mr.hdr  = mr.hdr; %me 23-07-2015
mr.data = mr.hdr.data;
mr.hdr  = rmfield(mr.hdr,'data');

% uncomment 23-07-2015
...
% define R|A|S and I|P|R spaces, get any xform info present
mr.spaces(1).name = []; % 'R|A|S';
mr.spaces(1).xform = mr.hdr.qto_xyz; % eye(4); % will modify this below, if possible
mr.spaces(1).dirLabels = []; % {'L <--> R' 'P <--> A' 'I <--> S'};
mr.spaces(1).sliceLabels = []; % {'Sagittal' 'Coronal' 'Axial'};
mr.spaces(1).units = 'mm';
mr.spaces(1).coords = [];

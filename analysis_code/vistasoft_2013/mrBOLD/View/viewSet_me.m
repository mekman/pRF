function vw = viewSet_me(vw,param,val,varargin)
...
switch param
    ...
    case 'curdt'
        ...
        if ischar(val)
            ...
            for dt =1:length(dataTYPES) %#ok<*NODEF>
                if strcmpi(val, dataTYPES(dt).name), vw = viewSet_me(vw, 'curdt', dt); end
                match = true;
            end
    ...
    case 'anatinitialize'
        ...
        vw = viewSet_me(vw,'Anatomy Nifti', niftiRead(val));
        %Calculate Voxel Size as that is not read in
        vw = viewSet_me(vw,'Anatomy Nifti', niftiSet(viewGet(vw,'Anatomy Nifti'),'Voxel Size',prod(niftiGet(vw.anat,'pixdim'))));
        %Let us also calculate and and apply our transform
        %vw = viewSet_me(vw,'Anatomy Nifti',niftiApplyAndCreateXform(viewGet(vw,'Anatomy Nifti'),'Inplane'));
    case 'anatomynifti'
    ...
    case {'mesh' 'currentmesh' 'allmeshes' 'addmesh' 'meshdata' ...
        ...
        switch lower(param)
            case 'mesh'
                ...
                if isempty(val), vw = viewSet_me(vw, 'deleteMesh', whichMesh);
            ...
            case 'addmesh'
                ...
                % select the new mesh
                vw = viewSet_me(vw, 'currentmeshn', newMeshNum);
            ...
            case 'deletemesh'
                ...
                if length(val) > 1
                    ...
                    for ii = 1:length(meshList)
                        ...
                        vw = viewSet_me(vw, 'deleteMesh', whichMesh);
                    end
                    return
                end
                ...
                if ~isempty(vw.mesh), 
                    vw = viewSet_me(vw, 'currentmeshn', max(1,min(val)-1));
                end
                ...
                if isempty(vw.mesh)
                    vw = rmfield(vw, 'mesh');
                    vw = viewSet_me(vw,'currentmeshn',0);
                end

function ok = mrInit_updateInplaneSession_me()
%
%
...
try
    ...
    load(fullfile(pwd,'mrSESSION.mat'));
...
%     xform = [diag(1./mrSESSION.inplanes.voxelSize), size(inplaneAnat.anat)'/2; 0 0 0 1];
    %xformPath = fullfile(pwd,'xform.txt');
    %xform = textread('xform.txt');
    
    xformPath = fullfile(pwd,'Raw/Scan1/Scan1-0000.nii');
    hdr = readFileNifti(xformPath);
    xform = hdr.qto_xyz;
